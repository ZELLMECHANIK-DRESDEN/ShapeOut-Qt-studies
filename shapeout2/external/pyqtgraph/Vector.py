...
class Vector(QtGui.QVector3D):
    ...
    def __init__(self, *args):
        if len(args) == 1:
            if isinstance(args[0], QtCore.QSizeF):
                QtGui.QVector3D.__init__(self, float(
                    args[0].width()), float(args[0].height()), 0)
                return
            elif isinstance(args[0], QtCore.QPoint) or isinstance(args[0], QtCore.QPointF):
                QtGui.QVector3D.__init__(self, float(
                    args[0].x()), float(args[0].y()), 0)
                ...
                if len(vals) != 3:
                    raise Exception(
                        'Cannot init Vector with sequence of length %d' % len(args[0]))
    ...
    def angle(self, a):
        ...
        ang = np.arccos(np.clip(QtGui.QVector3D.dotProduct(
            self, a) / (n1 * n2), -1.0, 1.0))  # in radians
